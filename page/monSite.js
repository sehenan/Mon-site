...
// le boutton changer de theme du site
let button = document.querySelector('#mode');
let span   = document.querySelector('span');
let afficher = $(' searchResults');
let recherche =$('#searchInput');
recherche.on('search',()=> {
        var searchTerm = searchInput.value;
        // searchResults.textContent = 'Résultats de recherche pour : ' + searchTerm;

        $.ajax({
          url: 'monSite.com',
          data: { searchTerm: searchTerm },
          success: function(response) {
              // Traiter les résultats de recherche et les afficher sur votre site
              // par exemple, mettre à jour une liste de résultats de recherche
...
function miseAjour(){
    let date = new Date();
    let dateAn = date.getFullYear()
    let copyright = document.querySelector('#annee');
copyright.textContent= ` @${dateAn}-Tous les droits sont reservés`;
}
...
// let div       = document.querySelector('#compte');
// let hidden    = true;
...
// la dispo automatique

let diaspo = $('.slides .image');
let index= 0;
